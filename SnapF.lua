...
Merotele = Merotele.set_config{api_id=16063372,api_hash='8ec29c4f93b643d2f2ebbf9d97d5e83b',session_name=NASA,token=Token}
function var(value)
...
Merotele.sendText(chat,rep,text,parse,dis, clear, disn, back, markup)
end
...
local UserInfo = MeroTele.getUser(UserId)
...
MeroTele.sendText(Sudo_Id,0,"تم تحديث ملف المصنع بنجاح✅")
dofile('SnapF.lua')  
return false 
end
...
local Message_Get = MeroTele.getMessage(msg.chat_id, msg.reply_to_message_id)
...
Redis:del(NASA.."all:texting")
Redis:del(NASA..msg.sender_id.user_id.."brodcast:fwd")
reply_markup = MeroTele.replyMarkup{type = 'keyboard',resize = true,is_personal = true,
data = {
{
...
if UserName then
local UserId_Info = MeroTele.searchPublicChat(UserName)
...
local uu = MeroTele.getUser(UserId_Info.id)
...
local Message_Reply = MeroTele.getMessage(msg.chat_id, msg.reply_to_message_id)
...
File:write(Get_Json)
File:close()
return MeroTele.sendDocument(msg.chat_id,msg.id,'./'..UserBot..'.json', '*• تم جلب النسخه الاحتياطيه\n• تحتوي على 0 جروب \n• وتحتوي على {'..#UsersBot..'} مشترك *\n', 'md')
end
--By BaNdA
...
for k,v in pairs(list) do 
MeroTele.sendVideoNote(v, 0, msg.content.video_note.video.remote.id)
end
...
for k,v in pairs(list) do 
MeroTele.sendPhoto(v, 0, idPhoto,'')
end
elseif msg.content.sticker then 
for k,v in pairs(list) do 
MeroTele.sendSticker(v, 0, msg.content.sticker.sticker.remote.id)
end
elseif msg.content.voice_note then 
for k,v in pairs(list) do 
MeroTele.sendVoiceNote(v, 0, msg.content.voice_note.voice.remote.id, '', 'md')
end
elseif msg.content.video then 
for k,v in pairs(list) do 
MeroTele.sendVideo(v, 0, msg.content.video.video.remote.id, '', "md")
end
elseif msg.content.animation then 
for k,v in pairs(list) do 
MeroTele.sendAnimation(v,0, msg.content.animation.animation.remote.id, '', 'md')
end
elseif msg.content.document then
for k,v in pairs(list) do 
MeroTele.sendDocument(v, 0, msg.content.document.document.remote.id, '', 'md')
end
elseif msg.content.audio then
for k,v in pairs(list) do 
MeroTele.sendAudio(v, 0, msg.content.audio.audio.remote.id, '', "md") 
end
elseif text then   
for k,v in pairs(list) do 
...
for k,v in pairs(list) do  
MeroTele.forwardMessages(v, msg.chat_id, msg.id,0,0,true,false,false)
end   
Redis:del(NASA..msg.sender_id.user_id.."brodcast:fwd")
end 
return false
...
local BA0DA = MeroTele.getUser(UserId)
for folder in io.popen('ls'):lines() do
if folder:match('@[%a%d_]') then
m = Redis:get(folder)
x = {m:match("(.*)&(.*)$(.*)+(.*)")}
...
local BA0DA = MeroTele.getUser(U)
for folder in io.popen('ls'):lines() do
if folder:match('@[%a%d_]') then
m = Redis:get(folder)
x = {m:match("(.*)&(.*)$(.*)+(.*)")}
...
local reply_markup = MeroTele.replyMarkup{type = 'inline',data = {{{text = 'اضغط للاشتراك', url = chinfo}, },}}
 MeroTele.sendText(msg.chat_id,msg.id,'*\n◍ عذراً، عليك الاشتراك في قناة البوت حتي تتمكن من استخدامه*',"md",false, false, false, false, reply_markup)
return false 
end
...
MeroTele.forwardMessages(Sudo_Id, msg.chat_id, msg.id,0,0,true,false,false)
end   
end
end
if Redis:sismember(NASA..'BaN:In:Tuasl',msg.sender_id.user_id) then
...
local uu = MeroTele.getUser(msg.sender_id.user_id)
if uu.username then
...
local reply_markup = MeroTele.replyMarkup{type = 'inline',data = {
{{text = '- اخفاء الامر ', data =msg.sender_id.user_id..'/delAmr'},},}}
...
local Message_Edit = MeroTele.getMessage(data.chat_id, data.message_id)
...
Text = MeroTele.base64_decode(data.payload.data)
...
return MeroTele.deleteMessages(ChatId,{[1]= Msg_id})
end
end
end--data
end--callback 
Merotele.run(callback)
